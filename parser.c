...
void lista_declaraciones_param(set folset)
{
    ...
    while (sbol->codigo ==CCOMA || in(sbol->codigo,first(DECLARACION_PARAMETRO)))
    {
...
void declarador_init(set folset)
{
    ...
    case CCOR_ABR:
    {
        ...
        if(flagExisteIDDeclaracion==1)
        {
            ...
            {
                ts[en_nivel_actual(ultimoID)].ets->desc.part_var.arr.cant_elem=-1;
            }
        }
        else
        {
...
void lista_inicializadores(set folset)
{
    ...
    constante(folsetLlamada);
    int cantElementosArreglos=1;

    while (sbol->codigo == CCOMA || sbol->codigo==CCONS_CAR || sbol->codigo==CCONS_ENT || sbol->codigo==CCONS_FLO  )
    {
        if(sbol->codigo==CCOMA)
            scanner();
        else
    ...
    int cantidad=ts[en_nivel_actual(ultimoID)].ets->desc.part_var.arr.cant_elem;
    if(cantidad!=-1)
    {
    ...
    {
        ts[en_nivel_actual(ultimoID)].ets->desc.part_var.arr.cant_elem=cantElementosArreglos;
    }
